...
const initialFormValues: CompanyProject = {
  name: "",
  url: "",
  description: "",
  projectsImages: [],
...
interface CompanyProjectFormProps {
  info: CompanyProject[];
  newInfoHandler: (values: CompanyProject, options: any) => Promise<any>;
  deleteInfoHandler: (id: string) => void;
  editInfoHandler: ((values: CompanyProject, options: any) => void) | null;
  editModeCallBack: ((updatedProjectInfo: CompanyProject) => void) | null;
...
const CompanyProjectForm = ({
  ...
  defaultListIcon,
  submitLoader = false,
}: CompanyProjectFormProps) => {
  const [initValues, setInitValues] =
    useState<CompanyProject>(initialFormValues);
  const windowWidth = useWindowWidth();

  const populateForm = (formData: CompanyProject) => {
    const images: File[] = [];
    if (formData?.projectsImages) {
      formData?.projectsImages.forEach((imgName: any) => {
        images.push(
          new File([], getLastSegment(imgName), { type: "image/png" }),
        );
      });
    }
    const updatedProjectInfo: CompanyProject = {
      ...formData,
      projectsImages: images,
    };
    setInitValues(updatedProjectInfo);
    if (editModeCallBack !== null) {
      editModeCallBack(updatedProjectInfo);
    }
  };
  ...
  return (
    <>
      ...
      <div className="row">
        {info.map(({ name, url, description, projectsImages, _id }, idx) => (
