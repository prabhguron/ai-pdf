...
contract MediationServiceBETA is VRFConsumerBaseV2, DataStructuresLibrary {
    ...
    uint256 public mediatorFlatFee = 0.001 ether; 
    ...
    error MediationService__DISABLED_FOR_BETA();
    ...
    function payMediationFee(uint256 _disputeId, string[] calldata _evidenceURIs) external payable {
        ...
        if (dispute.feePaidClaimant && dispute.feePaidRespondent) {
            ...
            _resolveDisputeBETA(_disputeId);
        }
    }

    function _resolveDisputeBETA(uint256 _disputeId) private {
        Dispute storage dispute = disputes[_disputeId];
        // check dispute ?
        // update phase
        dispute.phase = Phase.Decision;
        // resolve dispute
        bool granted = block.timestamp % 2 == 0;
        dispute.granted = granted;
        dispute.decisionRenderedDate = block.timestamp;
        // handle fees
        feesHeld[_disputeId] -= dispute.mediationFee; // half of fees held subtracted to mimic fees paid to mediators
        emit DecisionReached(_disputeId, dispute.granted, 0); // emits 0 for 'Majority Votes'
    }
    ...
    function submitAdditionalEvidence(uint256 _disputeId, string[] calldata _evidenceURIs) external {
        revert MediationService__DISABLED_FOR_BETA();
        // Dispute storage dispute = disputes[_disputeId];
        // if (msg.sender != dispute.claimant && msg.sender != dispute.respondent) revert MediationService__OnlyDisputant();
