...
const getUserProfiles = async (userRole, options) => {
  try {
    switch (userRole) {
      ...
      case "company":
        return await getCompanyProfiles(options);
      default:
        break;
    }
  } catch (error) {}
...
const getCompanyProfiles = async (options) => {
  let totalCount = 0;
  let allProfiles = [];
  try {
    let {
    ...
    let companyProfiles = CompanyProfile.find()
      .select({
        _id: 1,
        userId: 1,
        companyName: 1,
    ...
    //   conditions = { ...conditions, ...matchFilter };
    // }
    companyProfiles = companyProfiles.where(conditions);

    if (limit) {
      totalCount = await CompanyProfile.countDocuments(companyProfiles.getQuery());
      companyProfiles = companyProfiles.limit(parseInt(limit)).skip(parseInt(skip));
    }

    allProfiles = await companyProfiles.lean().exec();
